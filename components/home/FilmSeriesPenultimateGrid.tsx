...
type FilmSeriesPenultimateGridProps = {
  filmesSeriesArticles: Article[];
};

export default function FilmSeriesPenultimateGrid({
  filmesSeriesArticles,
}: FilmSeriesPenultimateGridProps) {
  const sorted = filmesSeriesArticles
  ...
  const penultimateFilmSeries = sorted.slice(0, 7);

  if (penultimateFilmSeries.length === 0) {
    return null;
  }

  const firstCard = penultimateFilmSeries[0] ?? null;
  const remainingCards = penultimateFilmSeries.slice(1, 7);
      ...
      >
        ...
        <Link href={`/artigos/${article.slug}`} className="block h-full">
          <div className="min-w-0 p-3">
            <p className="inline-block text-[11px] font-semibold uppercase tracking-wide text-slate-700 bg-rose-100 px-2 py-0.5 mb-1">
              TV, FILMES & SÉRIES
            </p>
