...
const EconomiaCard: React.FC<EconomiaCardProps> = ({ artigos }) => {
  ...
  const economiaMaisRecente = economia
    .slice()
    .sort((a, b) => {
  ...
  return (
    <div className="w-full max-w-xl mx-auto relative flex items-center h-[150px] md:h-[215px] bg-white rounded-2xl shadow border overflow-hidden">
      <span className="absolute top-3 left-3 z-10 bg-green-600 text-white text-xs font-semibold px-3 py-1 rounded-full">Economia</span>
      ...
      <div className="flex-1 flex flex-col justify-center px-2">
        <h3 className="text-base md:text-lg font-bold text-slate-900 line-clamp-2 mb-1">{economiaMaisRecente.title}</h3>
        {economiaMaisRecente.excerpt && (
          <p className="text-xs md:text-sm text-slate-600 line-clamp-2">{economiaMaisRecente.excerpt}</p>
        )}
      </div>
    </div>
  );
